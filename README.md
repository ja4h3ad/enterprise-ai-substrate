...
The workflow also has one deliberately narrow human-review path. Delta's agreement and Exhibit D contain conflicting 24-hour and 48-hour incident-notification clauses with no graph precedence edge. Deterministic assessment—not a model confidence score—marks the evidence unresolved and invokes a genuine LangGraph interrupt. `ContractGraphApplication.get_review(run_id)` returns a stable packet with competing clauses, exact provenance, conflict reasons, limits, and checkpoint identity. `resume_review(...)` accepts either a reasoned abstention or a reasoned controlling-evidence selection; it rejects unknown evidence and stale checkpoints before resumption and persists the before/after decision trail. The replay fixture is `replay/conflict.json`, and `tests/test_analyst_review.py` is the executable demonstration.
